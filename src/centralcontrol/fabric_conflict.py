...
class fabric:
    ...
    status_datatype = np.dtype({'names': ['index', 'message'], 'formats': [
                               'u4', h5py.special_dtype(vlen=str)], 'titles': ['Index', 'Message']})

    # this is an internal datatype to store the region of interest info
    roi_datatype = np.dtype({'names': ['start_index', 'end_index', 'description'], 'formats': [
                            'u4', 'u4', object], 'titles': ['Start Index', 'End Index', 'Description']})

    # measurement list: columns = v, i, timestamp, status
    m = np.array([], dtype=measurement_datatype)
    # status list: columns = corresponding measurement index, status message
    s = np.array([], dtype=status_datatype)
    # list defining regions of interest in the measurement list
    r = np.array([], dtype=roi_datatype)
    ...
    def connect(self, dummy=False, visa_lib='@py', visaAddress='GPIB0::24::INSTR', pcbAddress='10.42.0.54:23', motionAddress=None, lightAddress=None, visaTerminator='\n', visaBaud=57600, ignore_adapter_resistors=False):
        ...
        if dummy:
            ...
            self.sm = k2400(visa_lib=visa_lib, terminator=visaTerminator,
                            addressString=visaAddress, serialBaud=visaBaud)
    ...
    def hardwareTest(self, substrates_to_test):
        ...
        for chan in range(n_adc_channels):
            print('ADC channel {:} Counts: {:}'.format(
                chan, self.pcb.getADCCounts(chan)))
        ...
        for substrate in substrates_to_test:
            ...
            print(
                '{:s}\t<-- Substrate {:s} adapter resistor value in ohms (AIN{:d})'.format(r, substrate, chan))

        print("LED test mode active on substrate(s) {:s}".format(
            substrates_to_test))
            ...
            for pix in range(8):
                ...
                if self.pcb.pix_picker(substrate, pix+1):
                    ...
                    if not ready_to_sweep:  # setup the sourcemeter if this is our first pixel
                        ...
                        self.sm.setupSweep(
                            sourceVoltage=False, compliance=2.5, nPoints=101, start=sweepLow, end=sweepHigh)
                        # this allows for the trigger style we'll use here
                        self.sm.write(':arm:source bus')
                        ready_to_sweep = True
    ...
    def runSetup(self, operator, diode_cal, ignore_diodes=False, run_description=''):
        ...
        while True:
            ...
            prefix_match = any([file.startswith(prefix)
        ...
        self.f.attrs['Control Software Revision'] = np.string_(
        ...
        print("Intensity = [{:0.4f} {:0.4f}] suns".format(
    ...
    def substrateSetup(self, position, suid='', variable_pairs=[], layout_name=''):
        ...
        if self.pcb.pix_picker(position, 0):
            ...
            self.f[position].attrs['Sample Unique Identifier'] = np.string_(
                suid)

            #self.f[position].attrs['Sample Adapter Board Resistor Value'] = self.pcb.resistors[position]
            self.f[position].attrs['Sample Layout Name'] = np.string_(
                layout_name)
            for pair in variable_pairs:  # attach the user defined name-value pairs to each substrate
                ...
                self.f[position].attrs['User_' +
                                       parameter_name] = np.string_(parameter_value)
    ...
    def pixelSetup(self, pixel, t_dwell_voc=10, voltage_compliance=2):
        ...
        if self.pcb.pix_picker(pixel[0][0], pixel[0][1]):
            ...
            self.f[self.position+'/' +
                   self.pixel].attrs['area'] = self.area * 1e-4  # in m^2

            vocs = self.steadyState(t_dwell=t_dwell_voc, NPLC=10, sourceVoltage=False,
                                    compliance=voltage_compliance, senseRange='a', setPoint=0)
    ...
    def pixelComplete(self):
        ...
        # reset status storage
        self.s = np.array([], dtype=self.status_datatype)
        # reset region of interest
        self.r = np.array([], dtype=self.roi_datatype)
        self.Voc = None
        self.Isc = None
        self.mppt.reset()
    ...
    def registerMeasurements(self, measurements, description):
        ...
        roi['area'] = self.area
        try:
            # send the new region of interest data to the GUI
            self.update_gui(roi)
        except:
        ...
        if length > 0:
            stop = len(self.m) - 1
            start = stop - length + 1
            print("New region of interest: [{:},{:}]\t{:s}".format(
    ...
    def steadyState(self, t_dwell=10, NPLC=10, sourceVoltage=True, compliance=0.04, setPoint=0, senseRange='f'):
        ...
        """
        self.insertStatus('Measuring steady state {:s} at {:.0f} m{:s}'.format(
            'current' if sourceVoltage else 'voltage', setPoint*1000, 'V' if sourceVoltage else 'A'))
        if NPLC != -1:
            self.sm.setNPLC(NPLC)
        self.sm.setupDC(sourceVoltage=sourceVoltage, compliance=compliance,
                        setPoint=setPoint, senseRange=senseRange)
    ...
    def sweep(self, sourceVoltage=True, senseRange='f', compliance=0.04, nPoints=1001, stepDelay=0.005, start=1, end=0, NPLC=1, message=None):
        ...
        self.sm.setNPLC(NPLC)
        self.sm.setupSweep(sourceVoltage=sourceVoltage, compliance=compliance, nPoints=nPoints,
                           stepDelay=stepDelay, start=start, end=end, senseRange=senseRange)
    ...
    def track_max_power(self, duration=30, message=None, NPLC=-1, extra="basic://7:10"):
        ...
        raw = self.mppt.launch_tracker(
            duration=duration, NPLC=NPLC, extra=extra)
        ...
        if (self.mppt.Impp != None) and (self.mppt.Vmpp != None):
            self.f[self.position+'/' +
                   self.pixel].attrs['ssPmax'] = abs(self.mppt.Impp * self.mppt.Vmpp)
